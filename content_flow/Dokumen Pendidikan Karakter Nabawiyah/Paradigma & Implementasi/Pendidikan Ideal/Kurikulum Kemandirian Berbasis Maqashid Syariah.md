---
title: 'Alur Materi: Kurikulum Kemandirian Berbasis Maqashid Syari''ah'
---

# Alur Materi: Kurikulum Kemandirian Berbasis Maqashid Syari'ah

> [!info] Artikel Lengkap
> Berkas materi lengkap: [[content/Paradigma - Implementasi PKN/Dokumen Pendidikan Karakter Nabawiyah/Paradigma & Implementasi/Pendidikan Ideal/Kurikulum Kemandirian Berbasis Maqashid Syariah|Kurikulum Kemandirian Berbasis Maqashid Syari'ah]]

```mermaid
flowchart TD
    subgraph S1["1. Fondasi & Hakikat"]
        N1_1["1. Hakikat Maqashid Syari'ah dalam Pendidikan<br/>Nabawiyah<br/>- Landasan Otoritatif Turats: Imam Al-Ghazali<br/>& Imam Asy-Syathibi"]
    end

    subgraph S2["2. Dinamika & Prinsip"]
        N2_1["2. Pemetaan Kurikulum Kemandirian Berbasis 5<br/>Maqashid Syari'ah<br/>- Matriks Komparasi 5 Maqashid dalam Desain<br/>Kurikulum PKN"]
        N2_2["4. Tiga Tingkatan Standar Kompetensi dalam PKN<br/>- Standar Syariat:<br/>- Standar yang telah ditetapkan secara qath'i oleh wahyu<br/>(Al-Qur'an dan As-Sunnah)."]
        N2_1 --> N2_2
    end

    subgraph S3["3. Metodologi & Penerapan"]
        N3_1["5. Kaidah Rekonstruksi Prioritas Kurikulum: Batas<br/>Cukup vs Pendalaman"]
    end

    subgraph S4["4. Evaluasi & Output"]
        N4_1["6. Integrasi 3 Rukun Bakat Nabawiyah Lintas Fase<br/>Usia<br/>- Rukun 1: Suka (Al-Hirsh) — Etape Thufulah (0–7 Tahun)<br/>- Rukun 2: Bisa (Al-Itqan) — Etape Tamyiz (7–10 Tahun)"]
        N4_2["7. Muara Akhir: Kesadaran Beretika dengan Target<br/>Akhirat<br/>- Ibadah ritual menjadi kenikmatan kalbu (Lazzatul Ibadah).<br/>- Kemandirian mencari nafkah menjadi ibadah 'amalul yad<br/>pemakmur bumi."]
        N4_1 --> N4_2
    end

    S1 --> S2
    S2 --> S3
    S3 --> S4
```
